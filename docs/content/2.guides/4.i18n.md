...
The `WebSite` node's `name` and `description` are read from your [Site Config](/docs/site-config/guides/how-it-works) and resolved per request during SSR based on the current locale.
