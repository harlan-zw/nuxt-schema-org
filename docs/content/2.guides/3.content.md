...
relatedPages:
  - path: /docs/schema-org/guides/i18n
    title: Nuxt I18n
...
To ensure the tags actually get rendered, you need to ensure you're using the `useHead()`{lang="ts"} composable with the `head` key.
...
**Using Nuxt Content v2?** No extra setup is required. Simply add the `schemaOrg` key to your frontmatter.
