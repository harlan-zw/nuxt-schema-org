...
relatedPages:
  - path: /docs/schema-org/guides/default-schema-org
    title: Default Schema.org
  - path: /docs/schema-org/guides/content
    title: Nuxt Content
  - path: /learn-seo/nuxt/mastering-meta/rich-results
    title: Rich Results
---
...
Put a stable site-wide identity in `nuxt.config.ts`:
...
Use `app.vue` or a page component only when the identity depends on runtime data. Static config is easier to validate and is enough for most sites.
...
- [`Person`{lang="ts"}](#when-should-i-use-person)
- [`Organization`{lang="ts"}](#when-should-i-use-organization)
- [`LocalBusiness`{lang="ts"}](#when-should-i-use-localbusiness)
...
Use `Person`{lang="ts"} when your website is about a person, personal brand, or personal blog.
...
Use `Organization`{lang="ts"} for companies, brands, non-profits, or communities. Also use it as a fallback when other options don't fit.
...
Use `LocalBusiness`{lang="ts"} for local businesses, stores, restaurants, or services with a physical address.
...
Use `OnlineStore`{lang="ts"} for ecommerce sites selling products online. `OnlineStore` is not a valid string value for the `identity` config option; you must use the `defineOrganization` composable and set `'@type': ['Organization', 'Store', 'OnlineStore']` explicitly.
